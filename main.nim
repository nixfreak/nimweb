...
import uri       # Encode urls: encodeUrl()
import code/database_utils # Utils used in database
import code/password_utils # File with password utils
...
proc checkLoggedIn(c: var TData) =
  ...
  # Get the users cookie name `sid`. If it does not exist, return
...
proc login(c: var TData, email, pass: string): tuple[b: bool, s: string] =
  ...
  for row in fastRows(db, query, toLowerAscii(email)):
    ...
    when isMainModule:
      ...
      # Connect to DB 
      try: 
        # We are using the values which we assigned ealier
...
routes:
  ...
  post "/dologin":
    ...
    if loginB:
      ...
      redirect("/login?msg=" & encoderUrl(loginS))
